...
if(isset($_POST['name3']) &&  isset($_POST['pass3']) && isset($_POST['email']))
  {
  	$sql="insert into login(Username,Password,Email)values(:name,ENCODE(:pass,'secret'),:email)";
  	$stmt=$pdo->prepare($sql);
  	$stmt->execute(array(':name' =>$_POST['name3'] ,':pass' =>$_POST['pass3'] ,':email'=>$_POST['email']));
  	$_SESSION['success']="Record edited";
  	header("location:home.html");
  	return;
